...
class oWebSocketServer extends ODBO
{
	...
	private function fread_stream($socket, $length, $timeout = 30)
	{
		...
		while (!feof($socket)) {
			...
			$request .= $new_content;
			if (strlen($new_content) === 0 && strlen($request) !== 0) {
				return $request;
			}
			$current = microtime(TRUE);
			if ($timeout <= $current - $start) {
				$this->debug("%s", "\tSocket read timed out.\n", "RedBold");
				return FALSE;
			}
			usleep(50000);
		}
